...
def top_k_top_p_filtering(
    ...
    args:
        ...
        top_p: float
            Value of cumulative probability of parameter highest probability tokens to keep for nucleus sampling
        filter_value: float
            Value to replace logits with if they are filtered
        min_tokens_to_keep: int
            Minimum number of tokens to keep in the logits
...
class VoiceCraft(nn.Module):
    def __init__(self, args):
        ...
        self.eog = nn.Parameter(torch.full((self.args.n_codebooks, 1), self.args.eog, dtype=torch.long),
        ...
        if self.args.eos > 0:
            ...
            self.eos = nn.Parameter(torch.full((self.args.n_codebooks, 1), self.args.eos, dtype=torch.long),
                                    requires_grad=False)  # [K 1]
        ...
        self.n_audio_tokens = [
                                  self.args.audio_vocab_size + self.args.n_special] * self.args.n_codebooks  # special tokens: empty token, EOG token, audio pad token
        ...
        for i in range(len(y)):
            if self.args.eos > 0:
                ...
                cur_y = [y[i, :, item[0]: item[1]] for item in non_mask_intervals[i][:-1]] + [
                    torch.cat([y[i, :, non_mask_intervals[i][-1][0]: non_mask_intervals[i][-1][1]], self.eos],
                              dim=-1)] + [torch.cat([y[i, :, item[0]: item[1]], self.eog], dim=-1) for item in
                                          mask_intervals[
                                              i]]  # only insert eog to the last non-mask-interval, which is when the utterance actual ends
            else:
                if reduced_eog:
                    cur_y = [y[i, :, item[0]: item[1]] for item in non_mask_intervals[i][:-1]] + [
                        torch.cat([y[i, :, non_mask_intervals[i][-1][0]: non_mask_intervals[i][-1][1]], self.eog],
                                  dim=-1)] + [torch.cat([y[i, :, item[0]: item[1]], self.eog], dim=-1) for item in
                                              mask_intervals[
                                                  i]]  # only insert eog to the last non-mask-interval, which is when the utterance actual ends
                else:
                    cur_y = [torch.cat([y[i, :, item[0]: item[1]], self.eog], dim=-1) for item in
                             non_mask_intervals[i]] + [torch.cat([y[i, :, item[0]: item[1]], self.eog], dim=-1) for item
                                                       in mask_intervals[
                                                           i]]  # eog is added to each section TODO this is not correct, I should add eog to non_mask_intervals if that segment is not the ending segment (as there is no way for the model to predict eog for those segments, and this will do harm to tts experiment, where the model randomly output eog for the first segment)
    ...
    def shift(self, rearranged_y):
        ...
        for i in range(len(rearranged_y)):
            ...
            out = [cur_pattern.build_pattern_sequence(z=cur_y.unsqueeze(0).contiguous(),
                                                      special_token=self.args.empty_token, keep_only_valid_steps=False)
                   for cur_pattern, cur_y in zip(cur_patterns, rearranged_y[i])]
            shifted_y.append(
                [item[0].squeeze(0) for item in out])  # the first item is values, later two are indexes and mask
        ...
        for i in range(len(shifted_y)):
            ...
            for j in range(len(shifted_y[i]) - 1):
                ...
                cur_mask_position.append(
                    sum([item.shape[1] for item in cur_inserted_y]))  # each item is of shape [K S], so take shape[1]
                cur_inserted_y.append(
                    self.eog)  # insert mask token of shape [K, 1], BUT we are actually using the eog token as a place holder here, as the real mask will be inserted in embed_y function
    ...
    def cat_y(self, inserted_y, mask_position, y_lens):
        ...
        assert cated_y.shape == torch.Size([max(new_y_lens), len(inserted_y),
                                            self.args.n_codebooks]), f"cated_y.shape: {cated_y.shape}, but it should be {torch.Size([max(new_y_lens, len(inserted_y), self.args.n_codebooks)])}"
    ...
    def prepare_input_target(self, y, y_lens):
        ...
        assert inserted_y[0][1].shape == torch.Size((self.args.n_codebooks,
                                                     1)), f"this should be a mask, so should have shape {(self.args.n_codebooks, 1)}, but it's {inserted_y[0][1].shape}"
    ...
    def revert_pattern(self, patterns, logits_use):
        ...
        for i in range(len(logits_use)):
            ...
            cur_logits_final_ret = [item[0].permute(0, 2, 3, 1).squeeze(0) for item in
                                    cur_logits_final]  # each element is of shape [K,T,card]
    ...
    def forward(self, batch):
        ...
        x = x[:,
            :x_lens.max()]  # this deal with gradient accumulation, where x_lens.max() might not be longer than the length of the current slice of x
        y = y[:, :, :y_lens.max()]
        ...
        logits = torch.stack([self.predict_layer[i](y_out) for i in range(self.args.n_codebooks)],
                             dim=1)  # [B K S card]
    ...
    def inference(
        ...
        Args:
          ...
          stop_repetition (`optional`) int
            ...
            if 1, will use kv cache, if 0, will not use kv cache
          silence_tokens (`optional`) list[int]
            a list of tokens that will be treated as silence tokens, and will not be generated
        ...
        shifted_y, patterns = self.shift(
            rearranged_y)  # each element [K S], patterns is not used, as we directly use the original input y
        ...
        more_mask_value = mask_value[0][
                          num_mask + 1:]  # NOTE this will be used in the generation loop for reference for inserting mask embedding
        new_y_lens[0] = mask_position[0][num_mask] + 2
        mask_position[0] = mask_position[0][:num_mask + 1]
        assert mask_position[0][num_mask] + 2 == cated_y.shape[
            1], f"num_mask: {num_mask}, mask_position: {mask_position}, cated_y.shape: {cated_y.shape}"
        ...
        past = torch.ones([self.args.num_decoder_layers, 2, x.shape[0]], device=x.device,
                          dtype=torch.float32) if kvcache else None
        ...
        def sample_helper(n_eog, logits, codebook_eog, top_k, top_p, temperature, prev_token, consec_silence_count,
                          stop_repetition, silence_tokens, cur_num_gen):
            if n_eog == 0:
                ...
                if (
                        samples[0, 0] == self.args.eog or torch.argmax(logits[0], dim=-1) == self.args.eog or
                        y_input.shape[1] > x_lens[0] * 10
        ...
        while True:
            ...
            logits = torch.stack([self.predict_layer[i](y_out) for i in range(self.args.n_codebooks)],
                                 dim=1)  # [B K S card], B==S==1, so [1 K 1 card]
            ...
            samples, codebook_eog, prev_token, consec_silence_count = sample_helper(n_eog, logits, codebook_eog, top_k,
                                                                                    top_p, temperature, prev_token,
                                                                                    consec_silence_count,
                                                                                    stop_repetition, silence_tokens,
                                                                                    cur_num_gen)
            cur_num_gen += 1
            cur_generated.append(samples.squeeze(-1))  # [K,1] -> [K]
            # get samples_emb
            samples_emb = torch.stack([self.audio_embedding[k](samples[k]) for k in range(self.args.n_codebooks)],
                                      dim=0)  # [K,1,D]
            ...
            if sum(codebook_eog) == self.args.n_codebooks:  # generation for the current span is done
                ...
                if len(more_mask_value) > 0:
                    ...
                    samples_emb = torch.cat([samples_emb, extra_emb],
                                            dim=1)  # [1,3,D] # prev_last_token, mask_token, empty token
                    assert samples_emb.shape == torch.Size(
                        (1, 3, self.args.d_model)), f"samples_emb.shape: {samples_emb.shape}"
                    ##################### silence repetition handling #####################
                    ##################### silence repetition handling #####################
                    consec_silence_count = 0
                    prev_token = None
                ...
                assert samples_emb.shape == torch.Size(
                    (1, 1, self.args.d_model)), f"samples_emb.shape: {samples_emb.shape}"
        ...
        for l, orig_span in enumerate(generated):
            ...
            assert unshifted_span.shape[1] == num_gen[
                l] - self.args.n_codebooks, f"len(unshifted_spans[0]): {len(unshifted_span[0])}, num_gen[l]: {num_gen[l]}"
            flatten_gen.append(unshifted_span)
        # logging.info(f"unshfited_span: {unshifted_span.shape}")
        # raise
        assert len(non_mask_intervals[0]) - 1 == len(
            flatten_gen), f"len(non_mask_intervals[0]): {len(non_mask_intervals[0])}, len(flatten_gen): {len(flatten_gen)}"
        res = []
        ...
        expected_y_len = y_len - sum([item[1] - item[0] for item in mask_intervals[0]]) + sum(
            [item - self.args.n_codebooks for item in num_gen])
        assert res.shape == torch.Size((1, self.args.n_codebooks,
                                        expected_y_len)), f"res.shape: {res.shape}, expected_y_len: {expected_y_len}. y_len - sum([item[1] - item[0] for item in mask_interval]) + sum([item - self.args.n_codebooks for item in num_gen]): {y_len}-{sum([item[1] - item[0] for item in mask_interval])} + {sum([item - self.args.n_codebooks for item in num_gen])}"
    ...
    def inference_tts(
        ...
        assert not (shifted_y[0][0][self.args.n_codebooks:] == self.args.empty_token).any() and not (
                    shifted_y[0][0][self.args.n_codebooks:] == self.args.eog).any(), shifted_y[0][0]
        ...
        def sample_helper(n_eog, logits, codebook_eog, top_k, top_p, temperature, prev_token, consec_silence_count,
            ...
            if n_eog == 0:
                ...
                if (
                        samples[0, 0] == eog_inference or torch.argmax(logits[0], dim=-1) == eog_inference or
                        y_input.shape[1] > x_lens[0] * (self.args.encodec_sr // 5)
                ):  # last one means y is already too long, shouldn't happen, but put it here
        ...
        assert res.shape == torch.Size((1, self.args.n_codebooks,
                                        expected_y_len)), f"res.shape: {res.shape}, expected_y_len: {expected_y_len}. y_len + sum([item - self.args.n_codebooks for item in num_gen]): {y_len} + {sum([item - self.args.n_codebooks for item in num_gen])}"
    ...
    def inference_tts_batch(
        ...
        while True:
            ...
            if cur_num_gen == 0:
                ...
                embedded_y = embedded_y.repeat(batch_size, 1,
                                               1)  # will be used to concat with newly generated token embedding
                ...
                assert x_input.shape[0] == batch_size and x_padding_mask.shape[0] == batch_size and y_input.shape[
                    0] == batch_size and new_y_lens.shape[
                           0] == batch_size, f"x_input.shape: {x_input.shape}, x_padding_mask.shape: {x_padding_mask.shape}, y_input.shape: {y_input.shape}, new_y_lens.shape: {new_y_lens.shape}"
            y_out, present = self.dec_forward(
                x_input,
                x_lens,
                x_attention_mask,
            ...
            y_out = y_out[:, -1:]  # only take the last token
            logits = torch.stack([self.predict_layer[i](y_out) for i in range(self.args.n_codebooks)],
                                 dim=1)  # [B K S card], S==1, so [B K 1 card]
            logits = logits.squeeze(2)  # [B K card]
            assert logits.shape == torch.Size(
                (batch_size, self.args.n_codebooks, self.n_audio_tokens[0])), f"{logits.shape}"

            n_eog = sum(codebook_eog)
            if self.args.eos > 0:
                for jj in range(self.args.n_codebooks):
                    logits[:, jj, self.args.eog] = -10000.
            samples, codebook_eog, prev_tokens, consec_silence_counts, keep = sample_helper(n_eog, logits, codebook_eog,
                                                                                            top_k, top_p, temperature,
                                                                                            prev_tokens,
                                                                                            consec_silence_counts,
                                                                                            stop_repetition,
                                                                                            silence_tokens, cur_num_gen,
                                                                                            keep)
